...
function! lsp#internal#completion#documentation#_enable() abort
    ...
    let s:Dispose = easycomplete#lsp#callbag#pipe(
        ...
        \       easycomplete#lsp#callbag#fromEvent('CompleteChanged'),
        \       easycomplete#lsp#callbag#filter({_->g:lsp_completion_documentation_enabled}),
        \       easycomplete#lsp#callbag#map({->copy(v:event)}),
        \       easycomplete#lsp#callbag#debounceTime(g:lsp_completion_documentation_delay),
        \       easycomplete#lsp#callbag#switchMap({event->
        \           easycomplete#lsp#callbag#pipe(
        \               s:resolve_completion(event),
        \               easycomplete#lsp#callbag#tap({managed_user_data->s:show_floating_window(event, managed_user_data)}),
        \               easycomplete#lsp#callbag#takeUntil(easycomplete#lsp#callbag#fromEvent('CompleteDone'))
        \           )
        \       })
        \   ),
        \   easycomplete#lsp#callbag#pipe(
        \       easycomplete#lsp#callbag#fromEvent('CompleteDone'),
        \       easycomplete#lsp#callbag#tap({_->s:close_floating_window(v:false)}),
        \   )
        \ ),
