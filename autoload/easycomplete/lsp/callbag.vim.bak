...
" empty() {{{
function! easycomplete#lsp#callbag#empty() abort
    let l:data = {}
...
" never() {{{
function! easycomplete#lsp#callbag#never() abort
...
" forEach() {{{
function! easycomplete#lsp#callbag#forEach(operation) abort
...
" interval() {{{
function! easycomplete#lsp#callbag#interval(period) abort
...
" delay() {{{
function! easycomplete#lsp#callbag#delay(period) abort
    let l:data = { 'period': a:period }
...
" take() {{{
function! easycomplete#lsp#callbag#take(max) abort
...
" skip() {{{
function! easycomplete#lsp#callbag#skip(max) abort
    let l:data = { 'max': a:max }
...
" map() {{{
function! easycomplete#lsp#callbag#map(F) abort
    let l:data = { 'f': a:F }
...
function! easycomplete#lsp#callbag#fromEvent(events, ...) abort
...
function! s:fromEventHandlerCallback(data) abort
    ...
    call a:data['sink'](1, easycomplete#lsp#callbag#undefined())
endfunction
...
" fromPromise() {{{
function! easycomplete#lsp#callbag#fromPromise(promise) abort
...
function! s:fromPromiseOnFulfilledCallback(data, ...) abort
    ...
    call a:data['sink'](1, a:0 > 0 ? a:1 : easycomplete#lsp#callbag#undefined())
    if a:data['ended'] | return | endif
    call a:data['sink'](2, easycomplete#lsp#callbag#undefined())
endfunction
...
" debounceTime() {{{
function! easycomplete#lsp#callbag#debounceTime(duration) abort
...
" throwError() {{{
function! easycomplete#lsp#callbag#throwError(error) abort
...
" of() {{{
function! easycomplete#lsp#callbag#of(...) abort
...
" fromList() {{{
function! easycomplete#lsp#callbag#fromList(list) abort
...
" concat() {{{
function! easycomplete#lsp#callbag#concat(...) abort
    let l:data = { 'sources': a:000 }
...
function! s:concatSourceCallback(data, t, d) abort
    ...
    elseif a:t == 2 && a:d != easycomplete#lsp#callbag#undefined()
        call a:data['sink'](2, a:d)
    elseif a:t == 2 
        let a:data['i'] = a:data['i'] + 1
        call a:data['next']()
...
" combine() {{{
function! easycomplete#lsp#callbag#combine(...) abort
    let l:data = { 'sources': a:000 }
...
function! s:combineFactory(data, start, sink) abort
    ...
    let a:data['vals'] = s:createArrayWithSize(a:data['n'], easycomplete#lsp#callbag#undefined())
    let a:data['sourceTalkbacks'] = s:createArrayWithSize(a:data['n'], easycomplete#lsp#callbag#undefined())
...
function! s:combineSourceCallback(data, i, t, d) abort
    ...
    elseif a:t == 1
        ...
        if l:_Nd == 0
            let l:arr = s:createArrayWithSize(a:data['n'], easycomplete#lsp#callbag#undefined())
            let l:j = 0
            while l:j < a:data['n']
...
function! easycomplete#lsp#callbag#distinctUntilChanged(...) abort
...
" takeUntil() {{{
function! easycomplete#lsp#callbag#takeUntil(notfier) abort
...
function! s:takeUntilSourceCallback(data, t, d) abort
    ...
    if a:t == 2
        call a:data['notifierTalkback'](2, easycomplete#lsp#callbag#undefined())
    endif
...
function! s:takeUntilNotifierCallback(data, t, d) abort
    if a:t == 0
        ...
        call a:data['notifierTalkback'](1, easycomplete#lsp#callbag#undefined())
        return
    endif
    if a:t == 1
        let a:data['done'] = 0
        call a:data['notifierTalkback'](2, easycomplete#lsp#callbag#undefined())
        call a:data['sourceTalkback'](2, easycomplete#lsp#callbag#undefined())
        if a:data['inited'] | call a:data['sink'](2, easycomplete#lsp#callbag#undefined()) | endif
        return
    endif
    if a:t ==2
...
function! s:takeUntilSinkCallback(data, t, d) abort
    ...
    if a:t == 2 && has_key(a:data, 'notifierTalkback') && a:data['notifierTalkback'] != 0 | call a:data['notifierTalkback'](2, easycomplete#lsp#callbag#undefined()) | endif
    call a:data['sourceTalkback'](a:t, a:d)
endfunction
" }}}

" takeWhile() {{{
function! easycomplete#lsp#callbag#takeWhile(predicate) abort
...
" group() {{{
function! easycomplete#lsp#callbag#group(n) abort
    let l:data = { 'n': a:n }
...
" flatten() {{{
function! easycomplete#lsp#callbag#flatten() abort
...
function! s:flattenTalkbackCallback(data, t, d) abort
    ...
    if a:t == 2
        if a:data['innerTalkback'] != 0 | call a:data['innerTalkback'](2, easycomplete#lsp#callbag#undefined()) | endif
        call a:data['outerTalkback'](2, easycomplete#lsp#callbag#undefined())
    endif
endfunction
...
function! s:flattenInnerSourceCallback(data, t, d) abort
    if a:t == 0
        ...
        call a:data['innerTalkback'](1, easycomplete#lsp#callbag#undefined())
    elseif a:t == 1
        call a:data['sink'](1, a:d)
    elseif a:t == 2 && !easycomplete#lsp#callbag#isUndefined(a:d)
        call a:data['outerTalkback'](2, easycomplete#lsp#callbag#undefined())
...
" scan() {{{
function! easycomplete#lsp#callbag#scan(reducer, seed) abort
...
" reduce() {{{
function! easycomplete#lsp#callbag#reduce(reducer, seed) abort
    let l:data = { 'reducer': a:reducer, 'seed': a:seed }
...
" switchMap() {{{
function! easycomplete#lsp#callbag#switchMap(makeSource, ...) abort
...
function! s:switchMapInputSourceCallback(data, t, d) abort
    ...
    if a:t == 1
        if has_key(a:data, 'currSourceTalkback')
            call a:data['currSourceTalkback'](2, easycomplete#lsp#callbag#undefined())
...
function! s:switchMapCurrSourceCallback(data, t, d, currT, currD) abort
    ...
    if (a:currT == 0 || a:currT == 1) && has_key(a:data, 'currSourceTalkback')
        call a:data['currSourceTalkback'](1, easycomplete#lsp#callbag#undefined())
    endif
    if a:currT == 2
        call remove(a:data, 'currSourceTalkback')
...
" materialize() {{{
function! easycomplete#lsp#callbag#materialize() abort
    let l:data = {}
...
function! s:materializeFSourceCallback(data, t, d) abort
    if a:t == 1
        call a:data['sink'](1, easycomplete#lsp#callbag#createNextNotification(a:d))
    elseif a:t == 2
        call a:data['sink'](1, easycomplete#lsp#callbag#isUndefined(a:d)
                    \ ? easycomplete#lsp#callbag#createCompleteNotification()
                    \ : easycomplete#lsp#callbag#createErrorNotification(a:d))
        call a:data['sink'](2, easycomplete#lsp#callbag#undefined())
    else
        call a:data['sink'](a:t, a:d)
    endif
...
" Notifications {{{
function! easycomplete#lsp#callbag#createNextNotification(d) abort
    return { 'kind': 'N', 'value': a:d }
endfunction

function! easycomplete#lsp#callbag#createCompleteNotification() abort
    return { 'kind': 'C' }
endfunction

function! easycomplete#lsp#callbag#createErrorNotification(d) abort
    return { 'kind': 'E', 'error': a:d }
endfunction

function! easycomplete#lsp#callbag#isNextNotification(d) abort
    return a:d['kind'] ==# 'N'
endfunction

function! easycomplete#lsp#callbag#isCompleteNotification(d) abort
    return a:d['kind'] ==# 'C'
endfunction

function! easycomplete#lsp#callbag#isErrorNotification(d) abort
    return a:d['kind'] ==# 'E'
endfunction
" }}}

" spawn {{{
" let s:Stdin = easycomplete#lsp#callbag#makeSubject()
" call easycomplete#lsp#callbag#spawn(['bash', '-c', 'read i; echo $i'], {
"   \ 'stdin': s:Stdin,
"   \ 'stdout': 0,
"   \ 'stderr': 0,
"   \ 'exit': 0,
...
"   \ })
"   call s:Stdin(1, 'hi')
"   call s:Stdin(2, easycomplete#lsp#callbag#undefined()) " requried to close stdin
function! easycomplete#lsp#callbag#spawn(cmd, ...) abort
    let l:data = { 'cmd': a:cmd, 'opt': a:0 > 0 ? copy(a:000[0]) : {} }
    return easycomplete#lsp#callbag#create(function('s:spawnCreate', [l:data]))
endfunction
...
function! s:spawnCreate(data, next, error, complete) abort
    ...
    if has_key(a:data['opt'], 'stdin')
        let a:data['stdinDispose'] = easycomplete#lsp#callbag#pipe(
            \ a:data['opt']['stdin'],
