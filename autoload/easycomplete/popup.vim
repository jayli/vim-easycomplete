...
function! s:InputCallback(...)
  call s:log('term callback', string(a:000))
endfunction

function! easycomplete#popup#input()
  let title = "test context"
  let opt = {
        \ 'filetype': "txt",
        \ 'col': 'cursor',
        \ 'border' :[1,1,1,1],
  ...
  let buf = term_start("tail -f " . easycomplete_root . "/assets/blank.file", {
  ...
  " noa silent call setbufline(buf, 1, [">>>>>>>","<<<<<<<<<<<<<<<<<<<<<<,"])
  ...
  call easycomplete#util#execute(winid, [
        \ 'tnoremap <expr> <CR> easycomplete#popup#PromptHandlerCR()', 
        \ 'tnoremap <expr> <ESC> easycomplete#popup#PromptHandlerESC()', 
        \ '',
        \ ''
        \ ])
  let g:t__winid = winid
  " call easycomplete#util#GotoWindow(winid)
endfunction

function! easycomplete#popup#PromptHandlerCR()
  call s:log("CR callback")
  return ""
endfunction

function! easycomplete#popup#PromptHandlerESC()
  call easycomplete#util#execute(g:t__winid, ["silent noa call feedkeys('\<C-C>')"])
  return ""
endfunction
