...
function! easycomplete#lsp#get_signature_help_under_cursor() abort
  let l:servers = filter(easycomplete#lsp#get_allowed_servers(), 'easycomplete#lsp#has_signature_help_provider(v:val)')
  if len(l:servers) == 0
    return
  endif
...
function! s:handle_signature_help(server, data)
  if easycomplete#lsp#client#is_error(a:data['response'])
    call easycomplete#lsp#utils#error('Failed to retrieve signature help information for ' . a:server)
    call easycomplete#signature#flush()
    return
  endif
  ...
  if !empty(a:data['response']['result']) && !empty(a:data['response']['result']['signatures'])
    ...
    call easycomplete#HandleLspSignature(a:server, l:contents)
